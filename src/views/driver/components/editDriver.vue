<template>
  <el-dialog v-model="dialogVisible" title="修改司机" width="60%" top="10px" center class="editDriver"
...
import {getDriverFullInfoByDriverId, editDriver} from "@/api/api.driver";
...
// region 查询数据
async function load(driverId = "") {
  loading.value = true;
  form.value = await getDriverFullInfoByDriverId(driverId);
  loading.value = false;
}
...
const submit = (formRef) => {
  ...
  formRef.validate((valid, errorFields) => {
    ...
    if (valid) {
      loading.value = true;
      editDriver(form.value).then(res => {
