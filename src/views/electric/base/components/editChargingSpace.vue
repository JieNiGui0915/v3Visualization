<template>
    <el-dialog v-model="dialogVisible" title="编辑充电车位" width="600" center top="10px" class="editChargingSpace"
        ...
        <el-form :model="form" :rules="rules" label-width="140" ref="formRef" status-icon v-loading="loading">
            ...
            <el-form-item label="充电枪" prop="pileGunIds">
                <el-select v-model="form.pileGunId" placeholder="选择充电枪" clearable v-loading="loadingPileGunList"
...
const open = (item) => {
    ...
    form.value.pileIdList = item.pileIds.split(",").map(id => id.trim());
    form.value.pileGunIdList = item.pileGunIds.split(",").map(id => id.trim());
    loadStationList();
};
defineExpose({
    open
...
const submit = (formRef) => {
    ...
    formRef.validate((valid) => {
        if (valid) {
            loading.value = true;
            chargingSpaceApi.editChargingSpace(form.value).then(res => {
