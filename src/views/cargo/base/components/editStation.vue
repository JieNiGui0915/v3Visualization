<template>
    <el-dialog v-model="dialogVisible" title="编辑场站" width="60%" center top="10px" class="editStation"
        ...
        <el-form :model="form" :rules="rules" label-width="140" ref="formRef" status-icon v-loading="loading">
            <div class="flex_row">
                <div class="flex-1">
                    ...
                    <el-form-item label="货场简称" prop="goodsYardShortName">
...
const open = (item) => {
    ...
    if (data.openingTime) {
        data.openingTime = "2000/01/01 " + data.openingTime;
    }
    if (data.closingTime) {
        data.closingTime = "2000/01/01 " + data.closingTime;
    }
...
const submit = (formRef) => {
    ...
    formRef.validate((valid) => {
        if (valid) {
            ...
            stationApi.editStation(form.value).then(res => {
