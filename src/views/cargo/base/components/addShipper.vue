<template>
    <el-dialog v-model="dialogVisible" title="新增货主" width="60%" center top="10px" class="addShipper"
                ...
                <div class="flex-1">
                    ...
                    <el-form-item label="主体类型" prop="subjectType">
                        <el-select v-model="form.subjectType" placeholder="" clearable>
                            <el-option :label="item.label" :value="item.value" v-for="item in subjectTypeList" :key="item.value"></el-option>
                        </el-select>
                    </el-form-item>
...
const submit = (formRef) => {
    ...
    formRef.validate((valid) => {
        if (valid) {
            loading.value = true;
            shipperApi.addShipper(form.value).then(res => {
