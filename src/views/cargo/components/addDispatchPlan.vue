<template>
  <el-dialog v-model="dialogVisible" title="添加派车计划" center top="30px" class="addDispatchPlan" width="50%"
      ...
      <div class="flex_row">
        <div class="flex-1">
          <el-form-item label="车队(可多选)" prop="fleetIds">
            <el-select v-model="form.fleetIds" multiple placeholder="选择车队(可多选)" clearable class="flex-1">
...
const open = async (item) => {
  ...
  form.value.planSite = order.loadAddress;
  form.value.planTime = order.beginTime;
  form.value.planVolume = order.freightVolume;
...
const submit = (formRef) => {
  ...
  formRef.validate((valid) => {
    if (valid) {
      loading.value = true;
      dispatchApi.addDispatchPlanBatch(form.value).then(res => {
